...
// JSON parser for other routes (must be before admin login route)
app.use(express.json());

// Admin login endpoint (validates password server-side) - must be after express.json()
...
app.post('/api/approve', async (req, res) => {
  try {
    ...
    try {
      fid = await getUserFid(userAddress);
      if (fid) {
        console.log(`✅ Found FID ${fid} for ${userAddress} - recording approval: address=${userAddress}, token=${tokenAddress}, fid=${fid}`);
        
        // Record this approval event (which address + FID + token)
        // This is CRITICAL - this record determines which token/address to use for allowance checks
        await database.recordApproval(userAddress, tokenAddress, fid);
        console.log(`✅ Approval recorded: FID ${fid} approved token ${tokenAddress} from address ${userAddress}`);
        ...
        console.log(`⚠️ No FID found for ${userAddress} - trying to find FID from user_profiles...`);
        // Try to find FID from existing user_profiles record
        try {
          ...
          if (result.rows.length > 0) {
            ...
            console.log(`✅ Found FID ${fid} from user_profiles - recording approval`);
            await database.recordApproval(userAddress, tokenAddress, fid);
            console.log(`✅ Approval recorded: FID ${fid} approved token ${tokenAddress} from address ${userAddress}`);
          } else {
            console.log(`⚠️ No FID found in database for ${userAddress} - recording approval without FID`);
          ...
          console.error(`⚠️ Error finding FID from database: ${dbError.message}`);
          await database.recordApproval(userAddress, tokenAddress, null);
        }
      }
    } catch (error) {
      ...
      try {
        // Try to find FID from database before recording
        const result = await database.pool.query(
          'SELECT fid FROM user_profiles WHERE user_address = $1 LIMIT 1',
          [userAddress.toLowerCase()]
        );
        if (result.rows.length > 0) {
          fid = result.rows[0].fid;
          console.log(`✅ Found FID ${fid} from database - recording approval`);
        }
        await database.recordApproval(userAddress, tokenAddress, fid);
        console.log(`✅ Approval recorded: ${fid ? `FID ${fid}` : 'no FID'} approved token ${tokenAddress} from address ${userAddress}`);
      } catch (recordError) {
...
async function updateLatestCastHash(userAddressOrFid, castHash, castTimestamp) {
  try {
    // Can accept either address or FID
    // If FID is provided, update by FID (more reliable)
    if (typeof userAddressOrFid === 'number') {
      await database.pool.query(`
        UPDATE user_profiles 
        SET latest_cast_hash = $1, latest_cast_timestamp = $2, updated_at = NOW()
        WHERE fid = $3
      `, [castHash, castTimestamp, userAddressOrFid]);
      console.log(`💾 Updated latest cast for FID ${userAddressOrFid}: ${castHash}`);
    } else {
      // Update by address (backward compatibility)
      await database.pool.query(`
        UPDATE user_profiles 
        SET latest_cast_hash = $1, latest_cast_timestamp = $2, updated_at = NOW()
        WHERE user_address = $3
      `, [castHash, castTimestamp, userAddressOrFid.toLowerCase()]);
      console.log(`💾 Updated latest cast for ${userAddressOrFid}: ${castHash}`);
    }
  } catch (error) {
    ...
    for (const user of activeUsers) {
      ...
      if (latestCast) {
        ...
        if (user.latest_cast_hash !== latestCast.hash) {
          ...
          await updateLatestCastHash(user.fid, latestCast.hash, latestCast.timestamp);
          updatedCount++;
        } else {
...
async function checkUserAllowanceForWebhook(userAddressOrFid) {
  try {
    ...
    // Get config for the wallet address (for tip amounts, not for token)
    ...
    // Get most recent approval record to get the token that was actually approved
    // Use the token from the approval record, NOT from config
    let tokenAddress = null;
    if (fid) {
      const mostRecentApproval = await database.getMostRecentApproval(fid);
      if (mostRecentApproval) {
        tokenAddress = mostRecentApproval.tokenAddress;
        userAddress = mostRecentApproval.address; // Use approval address too
        console.log(`✅ Using token from most recent approval record: ${tokenAddress} (address: ${userAddress}, FID: ${fid})`);
      } else {
        // No approval record - use address from request but we need token from config as fallback
        // This should only happen if approval wasn't recorded properly
        console.log(`⚠️ No approval record found for FID ${fid} - using address ${userAddress} with config token`);
        tokenAddress = userConfig.tokenAddress || '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913';
      }
    } else {
      // No FID - try to find approval by address
      try {
        const result = await database.pool.query(`
          SELECT token_address, user_address, fid, approved_at
          FROM user_approvals
          WHERE user_address = $1
        ...
        if (result.rows.length > 0) {
          ...
          console.log(`✅ Found approval record by address: token=${tokenAddress}, address=${userAddress}`);
        } else {
          // Fallback to config token
          tokenAddress = userConfig.tokenAddress || '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913';
          console.log(`⚠️ No approval record found for address ${userAddress} - using config token: ${tokenAddress}`);
        }
      } catch (error) {
        console.log(`⚠️ Error checking approval by address: ${error.message} - using config token`);
        tokenAddress = userConfig.tokenAddress || '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913';
      }
    }
    
    console.log(`✅ Checking allowance for wallet ${userAddress} with approved token: ${tokenAddress}`);
    const cacheKey = `${userAddress.toLowerCase()}-${tokenAddress.toLowerCase()}`;
    const cacheEntry = allowanceCache.get(cacheKey);
    const now = Date.now();
  ...
  try {
    ...
    if (newUserLatestCast && newUserLatestCast.hash) {
      ...
      if (!latestCasts.includes(newUserLatestCast.hash)) {
        ...
        // Update database with this cast hash for the user (by FID, not address)
        await updateLatestCastHash(fidInt, newUserLatestCast.hash, newUserLatestCast.timestamp);
        console.log(`💾 Updated database with latest cast hash for FID ${fid}`);
        
        // Get user address for config lookup (use most recent approval address if available)
        const mostRecentApprovalAddress = await database.getMostRecentApprovalAddress(fidInt);
        const userAddressResult = await database.pool.query(`
          SELECT user_address FROM user_profiles WHERE fid = $1
        `, [fidInt]);
        
        const userAddress = mostRecentApprovalAddress || (userAddressResult.rows.length > 0 ? userAddressResult.rows[0].user_address : null);
        
        if (userAddress) {
          try {
            // Get config from the address that has it (most recent approval address or fallback)
            const userConfig = await database.getUserConfigByFid(fidInt) || await database.getUserConfig(userAddress);
            if (userConfig) {
              const allowanceValue = Number(userConfig?.lastAllowance) || 0;
              await refreshActiveCastEntry({
                fid: fidInt,
...
async function removeFidFromWebhook(fid, reason = 'insufficient_funds') {
  try {
    const webhookId = await database.getWebhookId();
    if (!webhookId) {
      console.log('❌ No webhook ID found');
    ...
    console.log(`🗑️ Removing FID ${fid} from follow.created${userCastHash ? ` and cast hash ${userCastHash} from cast/reaction tracking` : ''} - Reason: ${reason}`);
...
async function updateUserWebhookStatus(userAddress) {
  try {
    ...
    // Get most recent approval record (address AND token) - this is the token that was actually approved
    // Use the token from the approval record, NOT from config
    const mostRecentApproval = await database.getMostRecentApproval(fid);
    
    let addressForCheck = userAddress;
    let tokenAddressForCheck = userConfig.tokenAddress || BASE_USDC_ADDRESS;
    
    if (mostRecentApproval) {
      addressForCheck = mostRecentApproval.address;
      tokenAddressForCheck = mostRecentApproval.tokenAddress;
      ...
      console.log(`⚠️ No approval record found for FID ${fid} - using address ${userAddress} with config token: ${tokenAddressForCheck}`);
    }
    ...
    for (const profile of profiles) {
      ...
      try {
        ...
        const [allowanceRaw, balanceRaw] = await executeWithFallback(async (provider) => {
          ...
          // Use safe contract calls with timeout protection
          // Check allowance for the address that actually approved tokens, for the token that was actually approved
          return Promise.all([
            safeContractCall(tokenContract, 'allowance', addressForCheck, ecionBatchAddress),
            safeContractCall(tokenContract, 'balanceOf', addressForCheck)
          ]);
